...
import { type Profile, type Option, type AnalyticsData, type KeyMetricsData } from "@/lib/types";
...
export default async function AnalyticsPage({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined }}) {
  ...
  // We fetch all videos and their related project data
  let query = supabase
    .from('videos')
    .select(`
      *,
  ...
  // --- NEW: Data Processing in JavaScript ---

  // 1. Calculate Key Metrics from the filtered videos
  const completedVideos = videos?.filter(v => v.status === 'Done') || [];
  const totalMinutes = completedVideos.reduce((acc, v) => acc + (v.duration_minutes || 0), 0);
  const totalSeconds = completedVideos.reduce((acc, v) => acc + (v.duration_seconds || 0), 0);
  ...
  // 2. Calculate Aggregated Data for Charts
  const getCategory = (video: any): string => {
    switch (groupBy) {
      case 'faculty': return video.projects?.faculties?.name || "N/A";
      case 'prodi': return video.projects?.prodi?.name || "N/A";
      case 'lecturer': return video.projects?.lecturers?.name || "N/A";
      case 'term': return video.projects?.terms?.name || "N/A";
      case 'editor': return video.profiles?.full_name || "Unassigned";
      default: return 'Overall';
    }
  };
  const aggregatedData = videos?.reduce((acc, video) => {
  ...
  const analyticsData = aggregatedData ? Object.values(aggregatedData) : [];
  
  // Fetch data for the filter dropdowns (unchanged)
