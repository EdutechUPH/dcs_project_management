...
export default function ProjectRequestForm({ terms, faculties, prodi }: DataProps) {
  ...
  useEffect(() => {
    if (selectedFaculty) {
      ...
      setSelectedProdi(''); setFilteredLecturers([]);
    } else { setFilteredProdi([]); }
  ...
  useEffect(() => {
    async function fetchLecturers() {
      if (selectedProdi) {
        ...
        setFilteredLecturers(lecturers as Lecturer[]);
        setIsLoadingLecturers(false);
      } else { setFilteredLecturers([]); }
    }
    fetchLecturers();
  }, [selectedProdi]);
  useEffect(() => {
    const count = Number(videoCount) || 0;
    const newTitles = Array.from({ length: count }, (_, i) => videoTitles[i] || `Video ${i + 1}${courseName ? ` - ${courseName}` : ''}`);
    setVideoTitles(newTitles);
  }, [videoCount]);
  useEffect(() => {
    const prevCourseName = prevCourseNameRef.current;
    const newTitles = videoTitles.map((title, index) => {
