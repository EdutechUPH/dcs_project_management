...
export default function EditProjectDetails({ project, masterLists }: EditProjectDetailsProps) {
  ...
  useEffect(() => {
    async function fetchLecturers() {
      if (selectedProdi) {
        ...
        const lecturers = await getLecturersByProdi(parseInt(selectedProdi));
      ...
      <form action={updateDetailsWithId} onSubmit={() => setIsEditing(false)}>
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => setIsEditing(false)} className="text-sm font-medium text-gray-600">Cancel</button>
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          ...
          <div>
            ...
            <input type="date" name="due_date" id="due_date" defaultValue={project.due_date} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2" />
          </div>
          <div>
  ...
  return (
    <div>
      ...
      <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        <DetailItem label="Due Date" value={new Date(project.due_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })} />
